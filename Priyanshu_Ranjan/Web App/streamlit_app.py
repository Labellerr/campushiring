...
with st.sidebar:
    ...
    model_choice = st.selectbox("YOLO Model", ["runs/train/my_yolo_model/weights/best.pt"])
...
if st.button("🚀 Start Tracking"):
    ...
    while True:
        ...
        for tid,(cx,cy,missed,tcls) in tracks.items():
            ...
            if chosen_box is not None and best_d<distance_threshold:
                ...
                cv2.putText(frame,f"{label} ID:{tid}",(x1,max(15,y1-6)),cv2.FONT_HERSHEY_SIMPLEX,0.6,color,2)
                ...
                cv2.putText(frame,f"{label} ID:{tid}",(int(cx)+8,int(cy)-8),cv2.FONT_HERSHEY_SIMPLEX,0.5,color,1)
    ...
    with open(outpath,"rb") as f:
        st.download_button("📥 Download Video", data=f.read(), file_name=f"tracked_{uploaded.name}", mime="video/mp4")
    json_path=os.path.join(tmpdir,"tracked_objects.json")
    with open(json_path,"w") as f:
        json.dump(json_data,f,indent=2)
    with open(json_path,"rb") as f:
        st.download_button("📥 Download JSON", data=f.read(), file_name="tracked_objects.json", mime="application/json")
